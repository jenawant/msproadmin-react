function transitionJsonToString(jsonObj: string, callback: (arg0: unknown) => void) {
  // 转换后的jsonObj受体对象
  let _jsonObj = null;
  ...
  if (Object.prototype.toString.call(jsonObj) !== '[object String]') {
    ...
    try {
      _jsonObj = JSON.stringify(JSON.parse(jsonObj.replace(/(\')/g, '"')));
    } catch (error) {
      // 转换失败错误信息
      console.error('您传递的json数据格式有误，请核对....');
      console.error(error);
...
export const formatJson = (jsonObj: string, callback: (arg0: unknown) => void = ()=>{}) => {
  // 转换后的字符串变量
  let formatted = '';
  // 换行缩进位数
  let pad = 0;
  ...
  // 存储需要特殊处理的字符串段
  const _index: { start: any; end: any }[] = [];
