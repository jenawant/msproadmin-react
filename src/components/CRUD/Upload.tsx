...
export default (props: {
  ...
  if (props.initialValue) {
    return props.fieldProps.type === 'image' ? <AntImage src={props.initialValue} /> : null;
  } else {
    return (
      <>
        {contextHolder}
        <ProFormUploadButton
          ...
          max={!props.fieldProps.multiple ? 1 : props.fieldProps?.max ?? undefined}
          accept={
          ...
          fieldProps={{
            ...
            listType: props.fieldProps.type === 'image' ? 'picture-card' : undefined,
            customRequest: async ({ file, onSuccess, onError }) => {
              ...
              const msg =
                props.fieldProps.type === 'image'
                  ? await common.uploadImage(formData, props?.hasWebToken ? true : false)
                  : await common.uploadFile(formData, props?.hasWebToken ? true : false);
              if (msg.success) {
                msg.data.url = tool.attachUrl(msg.data.url, storageMode[msg.data.storage_mode]);
                onSuccess?.(msg);
                props.fieldProps.onChange?.(
                  props.fieldProps.onlyData ? msg.data[props.fieldProps.returnType] : msg.data,
                );
              } else {
